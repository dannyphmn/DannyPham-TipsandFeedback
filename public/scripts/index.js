...
const validateTip = (newTip) => {
  ...
  const topicCheck = topic === 'UX' || topic === 'UI';
  if (!topicCheck) {
...
const submitDiagnostics = (submissionObj) => {
  fetch('/api/diagnostics', {
    ...
    .then((response) => response.json())
    .then((data) => {
      console.log('Diagnostics submitted:', data);
      showErrors(submissionObj.errors); // show alerts to user
    })
    .catch((error) => {
...
const handleFormSubmit = (e) => {
  ...
  const tipContent = document.getElementById('feedbackText').value;
